...
已修复：插件内部不再直接用原始 `session_id` 区分状态，而是生成带机器人身份的作用域键：

```text
platform:self_id:group_id:session_id
```

其中 `self_id` 是当前机器人账号，`platform` 是平台名称，`group_id` 用于区分群聊上下文。这样即使多个 NapCat 机器人收到来自同一用户或同一群的消息，也会被隔离到不同的防抖状态中。
...
`scoped_session_id` 的格式为 `platform:self_id:group_id:session_id`。这能隔离同一 AstrBot 实例下的多个 QQ/NapCat 机器人，避免不同机器人共用防抖状态。
